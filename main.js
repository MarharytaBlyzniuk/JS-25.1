 ...
 * Мета: Розробити функцію createDomElement, яка приймає назву тега, текстовий вміст та контейнер,
 * до якого потрібно додати новий елемент. Функція створює новий елемент з вказаним тегом та текстовим вмістом і додає цей елемент до заданого контейнера.
 *
 * Вимоги:
...
// Кодуємо ключ та значення для безпечного зберігання у cookie
    const encodedKey = encodeURIComponent(key);
    const encodedValue = encodeURIComponent(value);

    // Формуємо значення cookie у форматі "ключ=значення"
    const cookieValue = `${encodedKey}=${encodedValue}`;

    // Визначаємо час закінчення дії cookie (через 10 секунд)
    const expirationDate = new Date(Date.now() + 10000).toUTCString();

    // Встановлюємо cookie з ім'ям 'userInfo', значенням і часом закінчення
    document.cookie = `userInfo=${cookieValue}; expires=${expirationDate}; path=/`;

    // Виводимо повідомлення про успішне збереження cookie
    console.log(`Cookie 'userInfo' збережено з значенням: ${cookieValue}`);
}
 ...
 *  Перша функція повинна зберігати інформацію про користувача, а друга - отримувати її. Крім того,
 *  обидві функції повинні виводити відповідні повідомлення у консоль про успішне збереження або отримання даних.
 *
...
sessionStorage.setItem(key, value);  // Зберігаємо пару ключ-значення в sessionStorage
console.log(`Saved ${key}: ${value}`);  // Виводимо повідомлення про успішне збереження
}

function getUserInfo(key) {
 const value = sessionStorage.getItem(key);   // Отримуємо значення з sessionStorage
    console.log(`Retrieved ${key}: ${value}`);// Виводимо повідомлення про успішне отримання
    return value;
}
